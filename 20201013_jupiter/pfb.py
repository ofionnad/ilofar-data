...
if __name__ == "__main__":
    ...
    for idx, dataBlock in enuemrate(rawReader.readPlan(start = 0, nsamps = nTaps * nBranch * nWindowPerGulp)):
        for channel in dataBlock:
            dataBlock[channel, :] = pfb_spectrometer(dataBlock[channel, :], n_taps=nTaps, n_chan=nBranch, n_int=nInt, window_fn="hamming")
        ...
        plt.imshow(db(dataBlock.downsample(tfactor = 32)), cmap='viridis', aspect='auto')
        plt.colorbar()
        plt.xlabel("Channel")
        plt.ylabel("Time")
    plt.show()
